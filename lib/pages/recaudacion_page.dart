        ...
        return Theme(
          data: ThemeData.light().copyWith(
            colorScheme: ColorScheme.light().copyWith(
              primary:
                  Color(0xFF000080), // Cambiar el color principal a #000080
            ),
          ),
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          final recaudacionesList =
            ...
            Color? textColor; // Color opcional
            if (recaudacion.estadoRecaudacion.descripcion == "Validado") {
              textColor =
                  Colors.green; // Color verde si el estado es "Validado"
            } else if (recaudacion.estadoRecaudacion.descripcion ==
                "Rechazado") {
              textColor = Colors.red; // Color rojo si el estado es "Rechazado"
            }

            return Card(
              child: IgnorePointer(
                ignoring: textColor ==
                    null, // Ignorar si no se ha establecido un color
                child: InkWell(
                  onTap: () {
                    if (recaudacion.estadoRecaudacion.descripcion ==
                        "Validado") {
                      _navigateToDetailPage(
                          recaudacion); // Navegar a la página de detalles
                    }
                  },
                  child: ListTile(
                    title: Text(
                    ...
                    subtitle: Column(
                      ...
                      children: [
                        Text(
                          'Recaudación: ${double.parse(recaudacion.importe).toStringAsFixed(2)}',
                          style: TextStyle(color: textColor),
                        ),
                        Text(
                          'Fecha: ${DateFormat('dd-MM-yyyy').format(fecha)}',
          ...
          final data = [
            charts.Series<Recaudacion, String>(
              id: 'recaudaciones',
              colorFn: (_, __) => charts.Color.fromHex(code: '#000080'),
              domainFn: (Recaudacion recaudacion, _) =>
                  recaudacion.fechaOperacion,
              measureFn: (Recaudacion recaudacion, _) =>
                  double.parse(recaudacion.importe),
              data: validadoMonthRecaudaciones,
              labelAccessorFn: (Recaudacion recaudacion, _) =>
                  '\$${double.parse(recaudacion.importe).toStringAsFixed(2)}', // Display the exact number in the label
            ),
          ];
          ...
          final reciboMantData = [
            charts.Series<Recaudacion, String>(
              ...
              data: validadoMonthRecaudaciones,
              labelAccessorFn: (Recaudacion recaudacion, _) =>
                  '\$${double.parse(recaudacion.reciboMantRecaudacion.importe).toStringAsFixed(2)}',
            ),
          ];
          ...
          return Scaffold(
            ...
            body: Column(
              children: [
                ...
                Text(
                  'Recaudaciones', // Título "Recaudaciones"
                  style: TextStyle(
                ...
                Text(
                  'Balance de Pagos', // Título "Balance de Pagos"
                  style: TextStyle(
                    fontWeight: FontWeight.bold,
                    fontSize: 18,
                  ),
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(
                        'Total Recibo del mes: ${double.parse(totalReciboMant.toStringAsFixed(2))}',
                        style: TextStyle(color: Colors.white),
                      ),
                      Text(
                        'Total Recaudado: ${double.parse(totalRecaudado.toStringAsFixed(2))}',
                        style: TextStyle(color: Colors.white),
                      ),
                      Text(
                        'Diferencia: ${numberFormat.format(diferencia)}',
